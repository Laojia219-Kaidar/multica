...
export function reviewQueueDisposition(item: ReviewQueueItem): ReviewQueueDisposition {
  if (item.reviewStateReason || !item.reviewState) return "blocked";
  if (item.reviewState === "owner_decision") return "owner_decision";
  if (!item.reviewerAgentId || !item.reviewTargetTaskId) return "blocked";
  return "active";
}
    ...
    >
      ...
      {loading ? (
        ...
        <ol className="divide-y">
          {issues.map((issue) => {
            const disposition = reviewQueueDisposition(issue);
