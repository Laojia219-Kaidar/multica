...
function CockpitSectionRow({
  ...
  return (
    <div className="flex items-start gap-2 text-xs">
      <span className={"mt-0.5 inline-block size-1.5 shrink-0 rounded-full " + (healthy ? "bg-success" : "bg-destructive")} />
      <div className="min-w-0 flex-1">
        ...
        {section?.summary ? (
          ...
          <span className="ml-1 text-destructive" title={section.error}>不可达</span>
        ) : null}
      </div>
    </div>
  );
...
export function BasesPage() {
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      />
      {/* 与 Employees/Projects/Agents 表面对齐：头部同款 px-5 页面槽、
          4 单位区块间距、可滚动正文容器；卡片栅格保持既有响应式断点。 */}
        ...
        {companyBases.length > 0 && (
          <div className="rounded-lg border bg-card p-4 shadow-sm">
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              {companyBases.map((b) => (
                <span key={b.id} className="rounded-md border bg-muted/50 px-2 py-1 text-xs">
                ...
                <div key={machine.id} className="rounded-lg border bg-card shadow-sm">
                  ...
                  <div className="grid grid-cols-2 gap-2 px-4 pb-3 text-sm">
                    ...
                    <div>
                      ...
                      <div className={"font-medium " + (machine.onlineCount > 0 ? "text-success" : "text-destructive")}>
