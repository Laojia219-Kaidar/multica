...
if PATH="$tmp/bin:$PATH" HIVECREW_CANARY_MODE=1 HIVECREW_AUTH_TOKEN_REF=ref HIVECREW_WORK_ORDER=WO-TEST HIVECREW_QWEN_REAL_HOME="$tmp/home" HIVECREW_QWEN_BIN="$tmp/bin/qwen" HIVECREW_QWEN_SECRET_FILE="$tmp/home/.qwen/.env" HIVECREW_LANDLOCK_EXEC="$tmp/bin/landlock" CHAIN_LOG="$tmp/argv-auth" "$repo/ops/dgx-runtime-foundation/bin/qwen-chain" qwen-hive-qwen --auth-type oauth >/dev/null 2>&1; then exit 1; fi
