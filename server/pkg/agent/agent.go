...
// Supported types: "claude", "codebuddy", "codex", "copilot", "opencode", "deveco", "openclaw", "hermes", "pi", "cursor", "kimi", "kiro", "antigravity", "qoder", "traecli", "grok", "qwen", "mimo".
//
...
// native `qwen -p <prompt> --output-format stream-json` backend. mimo is the
// Xiaomi MiMo Code CLI ACP backend (`mimo acp`); session-login discovery is
// Ultra-only and this whitelist does not imply continuous dispatch is ready.
...
func New(agentType string, cfg Config) (Backend, error) {
	...
	default:
		return nil, fmt.Errorf("unknown agent type: %q (supported: claude, codebuddy, codex, copilot, opencode, deveco, openclaw, hermes, pi, cursor, kimi, kiro, antigravity, qoder, traecli, grok, qwen, mimo)", agentType)
	}
}
