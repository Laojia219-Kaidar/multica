...
// subcommand that drives the ACP JSON-RPC transport for MiMo Code CLI;
// overriding it would break the daemon↔MiMo communication contract.
var mimoBlockedArgs = map[string]blockedArgMode{
	"acp": blockedStandalone,
}

// mimoBackend implements Backend by spawning `mimo acp` and communicating
// via the ACP (Agent Client Protocol) JSON-RPC 2.0 over stdin/stdout.
//
// MiMo Code CLI exposes ACP via `mimo acp`. We reuse the existing hermesClient
// ACP transport since both runtimes speak the same protocol — only the
// binary, env, and tool-name extraction differ.
//
// Auth is session_login on Mac Studio Ultra only. This backend must not read
// or write vendor login secrets; login health comes from Ultra-side
// `mimo providers whoami` / session-login discovery, not from this process.
...
func (b *mimoBackend) Execute(ctx context.Context, prompt string, opts ExecOptions) (*Session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("mimo: invalid mcp_config: %w", err)
	}

	timeout := opts.Timeout
	runCtx, cancel := runContext(ctx, timeout)

	// `mimo acp` ignores --yolo / --auto-approve (they're flags on the
	// root `mimo` command, not on the `acp` subcommand). Instead, the
	...
	mimoArgs := append([]string{"acp"}, filterCustomArgs(opts.CustomArgs, mimoBlockedArgs, b.cfg.Logger)...)
	cmd := exec.CommandContext(runCtx, execPath, mimoArgs...)
	hideAgentWindow(cmd)
	b.cfg.Logger.Info("agent command", "exec", execPath, "args", mimoArgs)
	if opts.Cwd != "" {
		cmd.Dir = opts.Cwd
	}
	...
	if err != nil {
		cancel()
		return nil, fmt.Errorf("mimo stdin pipe: %w", err)
	}
	// Forward stderr to the daemon log *and* sniff provider-level
	// errors out of it so we can surface them in the task result.
	// MiMo's session/prompt still reports stopReason=end_turn when
	// the underlying HTTP call to the MiMo provider returns 4xx/5xx, so
	...
	providerErr := newACPProviderErrorSniffer("mimo")
	stderr, err := cmd.StderrPipe()
	if err != nil {
		cancel()
		return nil, fmt.Errorf("mimo stderr pipe: %w", err)
	}
	...
	stderrSink := io.MultiWriter(newLogWriter(b.cfg.Logger, "[mimo:stderr] "), providerErr)
	...
	b.cfg.Logger.Info("mimo acp started", "pid", cmd.Process.Pid, "cwd", opts.Cwd)

	msgCh := make(chan Message, 256)
	resCh := make(chan Result, 1)

	// MiMo streams interim narration and the final answer as the same
	...
	// Reuse the hermesClient ACP transport — MiMo speaks the same protocol.
	c := &hermesClient{
		cfg:          b.cfg,
		stdin:        stdin,
		...
		onMessage: func(msg Message) {
			...
			// (replace)", …) but not capitalised mimo-style ones
			// ("Read file: …", "Run command: …"). Re-normalise so
			// the UI sees consistent snake_case identifiers across
			// ACP backends. No-op when the name is already normal
			// form (e.g. already mapped to "read_file").
			if msg.Type == MessageToolUse {
				msg.Tool = mimoToolNameFromTitle(msg.Tool)
			}
	...
	go func() {
		...
		c.closeAllPending(fmt.Errorf("mimo process exited"))
	}()
	...
	go func() {
		...
		if err != nil {
			finalStatus = "failed"
			finalError = fmt.Sprintf("mimo initialize failed: %v", err)
		...
		// whole session/new.
		mcpServers = filterACPMcpServersByCapability(mcpServers, extractACPMcpCapabilities(initResult), "mimo", b.cfg.Logger)
		...
		if opts.ResumeSessionID != "" {
			...
			// this, a resumed MiMo task lost access to MCP tools that a
			...
			if err != nil {
				finalStatus = "failed"
				finalError = fmt.Sprintf("mimo session/resume failed: %v", err)
				resCh <- Result{Status: finalStatus, Error: finalError, DurationMs: time.Since(startTime).Milliseconds()}
				return
			}
			var changed bool
			...
			if err != nil {
				finalStatus = "failed"
				finalError = fmt.Sprintf("mimo session/new failed: %v", err)
				resCh <- Result{Status: finalStatus, Error: finalError, DurationMs: time.Since(startTime).Milliseconds()}
				return
			}
			...
			if sessionID == "" {
				finalStatus = "failed"
				finalError = "mimo session/new returned no session ID"
				resCh <- Result{Status: finalStatus, Error: finalError, DurationMs: time.Since(startTime).Milliseconds()}
				return
			}
		}
		...
		b.cfg.Logger.Info("mimo session created", "session_id", sessionID)

		// 3. If the caller picked a model (via agent.model from the
		// UI dropdown), ask mimo to switch the session to it before
		// we send any prompt. MiMo's ACP server exposes
		...
		// falling back to mimo's default model would let the user
		...
		if opts.Model != "" {
			if _, err := c.request(runCtx, "session/set_model", map[string]any{
				...
				b.cfg.Logger.Warn("mimo set_session_model failed", "error", err, "requested_model", opts.Model)
				finalStatus = "failed"
				finalError = fmt.Sprintf("mimo could not switch to model %q: %v", opts.Model, err)
			...
			b.cfg.Logger.Info("mimo session model set", "model", opts.Model)
		}
		...
		if err != nil {
			if runCtx.Err() == context.DeadlineExceeded {
				finalStatus = "timeout"
				finalError = fmt.Sprintf("mimo timed out after %s", timeout)
			} else if runCtx.Err() == context.Canceled {
				finalStatus = "aborted"
				finalError = "execution cancelled"
			} else {
				finalStatus = "failed"
				finalError = fmt.Sprintf("mimo session/prompt failed: %v", err)
				if opts.ResumeSessionID != "" && isACPSessionNotFound(err) {
			...
			case pr := <-promptDone:
				if pr.stopReason == "cancelled" {
					finalStatus = "aborted"
					finalError = "mimo cancelled the prompt"
				}
				c.usageMu.Lock()
		...
		b.cfg.Logger.Info("mimo finished", "pid", cmd.Process.Pid, "status", finalStatus, "duration", duration.Round(time.Millisecond).String())
...
// mimoToolNameFromTitle normalises tool names emitted by MiMo's ACP
// server into the snake_case identifiers the Multica UI expects.
//
// MiMo follows the ACP spec where `title` is a short human-readable
// label such as "Read file: /path/to/foo.go" or "Run command: ls".
// hermesToolNameFromTitle upstream handles hermes' lowercase
// convention ("read:", "patch (replace)") but not mimo's capitalised
...
func mimoToolNameFromTitle(title string) string {
