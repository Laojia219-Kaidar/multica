...
func NewRouterWithOptions(pool *pgxpool.Pool, hub *realtime.Hub, bus *events.Bus, analyticsClient analytics.Client, rdb *redis.Client, opts RouterOptions) (chi.Router, *handler.Handler) {
	...
	localOperatorSessionEnabled, localOperatorUserID := localOperatorSessionConfigFromEnv(context.Background(), queries)

	signupConfig := handler.Config{
		AllowSignup:                 os.Getenv("ALLOW_SIGNUP") != "false",
		AllowedEmails:               splitAndTrim(os.Getenv("ALLOWED_EMAILS")),
		AllowedEmailDomains:         splitAndTrim(os.Getenv("ALLOWED_EMAIL_DOMAINS")),
		LocalOperatorSessionEnabled: localOperatorSessionEnabled,
		LocalOperatorUserID:         localOperatorUserID,
	...
	if signupConfig.LocalOperatorSessionEnabled {
		r.With(authVerifyRL).Post("/auth/local-operator-session", h.StartLocalOperatorSession)
	}
	...
	r.Route("/api/daemon", func(r chi.Router) {
		...
		r.Post("/runtimes/{runtimeId}/tasks/{taskId}/writer-lease/{action}", h.WriterLease)
		...
		r.Group(func(r chi.Router) {
			...
			r.Route("/api/issues", func(r chi.Router) {
				...
				r.Route("/{id}", func(r chi.Router) {
					...
					// Lane A owner issue control plane (dispatch view). These routes
					// intentionally use the strict OwnerDispatchService handlers below;
					// do not register the legacy IssueService dispatch twins here.
