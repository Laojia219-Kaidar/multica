-- Lane B (P2): review/repair task kinds on the existing task queue — no second
-- task truth source is created.
--
...
-- delivery comment's source_task_id). The CHECK makes NULL bypass structurally
-- impossible for review rows: a task_kind='review' row MUST carry a candidate
-- reference, so the partial unique index below can never see NULL columns
-- (Postgres treats NULLs as distinct in unique indexes — this CHECK closes
-- that hole).
