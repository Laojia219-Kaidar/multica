...
func RejectForbiddenProofFields(raw []byte) error {
	...
	if key, ok := findForbidden(root, set, ""); ok {
		return fmt.Errorf("%w: %q", ErrForbiddenProofField, key)
	}
	return nil
}

func findForbidden(v any, set map[string]bool, path string) (string, bool) {
	switch t := v.(type) {
	case map[string]any:
		for k, val := range t {
			if set[k] {
			...
			if key, ok := findForbidden(val, set, joinPath(path, k)); ok {
				return key, true
			}
		}
	case []any:
		for i, e := range t {
			if key, ok := findForbidden(e, set, fmt.Sprintf("%s[%d]", path, i)); ok {
				return key, true
			}
		}
	}
