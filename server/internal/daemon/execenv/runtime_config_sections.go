...
func writeWorkflowIssue(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("**Review mode only — preserve the ReviewCell state machine**\n\n")
	b.WriteString("- Keep the Issue status exactly as you found it. Never run `multica issue status` in Review mode — not for in_progress, in_review, done, blocked, or any other status.\n")
	b.WriteString("- Inspect only the candidate Task and evidence named by the review handoff. Post exactly one Task-linked Agent Comment carrying the required `HIVECREW_REVIEW_VERDICT_V1` marker, and make the final assistant response repeat that identical marker as its final non-empty line.\n")
	b.WriteString("- The ReviewCell listener owns the canonical Issue transition after it verifies the completed Task and verdict. Never close, accept, requeue, or otherwise mutate the Issue status yourself.\n")
	b.WriteString("- If the review cannot be completed under that contract, report the blocker in the single required Task-linked Comment and final response without changing Issue status or fabricating a verdict marker.\n\n")
