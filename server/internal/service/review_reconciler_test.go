...
func TestReviewReconcilerMaxDispatchBoundsAttemptsAfterFailure(t *testing.T) {
	failing := reviewReconcileFixture(35)
	ready := reviewReconcileFixture(36)
	input := reviewReconcileInput(failing, ready)
	input.MaxDispatch = 1
	dispatcher := &reviewReconcileDispatcherFake{errs: map[string]error{failing.IssueID: errors.New("temporary worker conflict")}}
	result, err := NewReviewReconciler().Dispatch(context.Background(), input, "00000000-0000-0000-0000-000000000003", dispatcher)
	if err != nil {
		t.Fatalf("Dispatch: %v", err)
	}
	if len(dispatcher.calls) != 1 || dispatcher.calls[0] != failing.IssueID || len(result.Receipts) != 0 || result.Plan.Failed != 1 {
		t.Fatalf("calls=%v receipts=%d plan=%+v, want only first failed attempt", dispatcher.calls, len(result.Receipts), result.Plan)
	}
}

func TestReviewReconcilerPreCanceledContextMakesZeroCalls(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	dispatcher := &reviewReconcileDispatcherFake{}
	_, err := NewReviewReconciler().Dispatch(ctx, reviewReconcileInput(reviewReconcileFixture(45)), "00000000-0000-0000-0000-000000000003", dispatcher)
	if !errors.Is(err, context.Canceled) || len(dispatcher.calls) != 0 {
		t.Fatalf("err=%v calls=%v, want canceled and zero calls", err, dispatcher.calls)
	}
}

func TestReviewReconcilerExpiredDeadlineMakesZeroCalls(t *testing.T) {
	ctx, cancel := context.WithDeadline(context.Background(), time.Now().Add(-time.Second))
	defer cancel()
	dispatcher := &reviewReconcileDispatcherFake{}
	_, err := NewReviewReconciler().Dispatch(ctx, reviewReconcileInput(reviewReconcileFixture(46)), "00000000-0000-0000-0000-000000000003", dispatcher)
	if !errors.Is(err, context.DeadlineExceeded) || len(dispatcher.calls) != 0 {
		t.Fatalf("err=%v calls=%v, want deadline exceeded and zero calls", err, dispatcher.calls)
	}
}

func TestReviewReconcilerCancellationAfterFirstAttemptSkipsSecond(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	first, second := reviewReconcileFixture(47), reviewReconcileFixture(48)
	dispatcher := &reviewReconcileDispatcherFake{afterCall: cancel}
	_, err := NewReviewReconciler().Dispatch(ctx, reviewReconcileInput(first, second), "00000000-0000-0000-0000-000000000003", dispatcher)
	if !errors.Is(err, context.Canceled) || len(dispatcher.calls) != 1 || dispatcher.calls[0] != first.IssueID {
		t.Fatalf("err=%v calls=%v, want cancellation after one attempt", err, dispatcher.calls)
	}
}
