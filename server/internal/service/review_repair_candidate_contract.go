...
func repairCandidateHandoffNote(baseTaskID string) string {
	return fmt.Sprintf(
		"Repair only the exact base candidate Task %s. The current repair Task ID is supplied by the runtime task context. Do not reuse the old implementation comment or identity. Your final non-empty output line MUST be exactly %s followed by one JSON object with repair_task_id, base_task_id, base_candidate_revision, base_generation, candidate_revision, and generation. candidate_revision must change and generation must be the base numeric generation plus one. Missing, additional, malformed, duplicate, or non-final markers fail closed.",
