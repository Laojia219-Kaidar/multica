...
// ---------------------------------------------------------------------------
// Single Writer Lease with fencing generation (HIV-410 provisional candidate)
// -------------------------------------------------------------------
//
...
//
// INTEGRATION NOTE: migration 262 is provisional and unapplied. The table
// is created lazily by EnsureSchema when the service is first used in tests.
// In production, the migration must be promoted and applied before the
// service is wired into the task/worktree write path.
...
// ---------------------------------------------------------------------------
// SQL constants — mirror server/pkg/db/queries/write_lease.sql exactly.
// Inlined as consts so the service works without sqlc regeneration while
// migration 262 is provisional.
// ---------------------------------------------------------------------------

const writeLeaseSchemaDDL = `
...
CREATE INDEX IF NOT EXISTS idx_write_lease_status_expires
...
// EnsureSchema creates the write_lease table if it does not already exist.
// This is a no-op when migration 262 has been applied. The service calls
// this lazily on first use; callers may invoke it explicitly during setup.
func (s *WriteLeaseService) EnsureSchema(ctx context.Context) error {
	_, err := s.pool.Exec(ctx, writeLeaseSchemaDDL)
	return err
}
...
func (s *WriteLeaseService) Acquire(ctx context.Context, mutexKey, holderID string, ttl time.Duration) (*WriteLease, error) {
	if err := s.EnsureSchema(ctx); err != nil {
		return nil, fmt.Errorf("write lease ensure schema: %w", err)
	}
	if ttl <= 0 {
