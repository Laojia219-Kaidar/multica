...
// (keyed by the app name "HiveCrew Canary"). The env hooks to override both
...
// The dev app then shows e.g. "HiveCrew Canary hivecrew-3724-194" in Cmd+Tab and gets
...
// the primary checkout keeps the unchanged 5173 / "HiveCrew Canary" defaults.
  ...
  if (log) {
    console.log(
      `[dev:desktop] worktree isolation → renderer port ${env.DESKTOP_RENDERER_PORT}, ` +
        `app "HiveCrew Canary ${env.DESKTOP_APP_SUFFIX}"`,
    );
  }
  return env;
}
