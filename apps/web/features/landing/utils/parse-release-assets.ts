 ...
 *   hivecrew-desktop-{version}-mac-{arch}.{dmg|zip}
 *   hivecrew-desktop-{version}-windows-{arch}.exe
 *   hivecrew-desktop-{version}-linux-{arch}.{AppImage|deb|rpm}
 *
...
const DESKTOP_ARTIFACT_RE =
  /^(?:hivecrew|multica)-desktop-[^-]+-(mac|windows|linux)-([a-z0-9_]+)\.(dmg|zip|exe|AppImage|deb|rpm)$/i;
